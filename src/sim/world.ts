...
export function castAbility(
  ...
  const to = { ...aimPoint }
  const radius = slot === 'r' ? 72 : slot === 'w' ? 52 : u.archetype === 'mage' ? 22 : 28
...
/** R-click attack: ranged stands and AAs at range; melee still closes. */
export function issueAttack(u: Unit, target: Unit) {
  u.targetId = target.id
  u.attackMoveTo = null
  u.pendingWard = null
  u.moveTo = !u.stats.melee && inAaRange(u, target) ? null : { ...target.pos }
}
...
export function tickWorld(world: World) {
  ...
  if (world.warmup > 0) {
    world.warmup -= DT
    if (world.warmup <= 0) world.warmup = 0
    return
  }
